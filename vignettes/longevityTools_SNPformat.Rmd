...
## QC starting
Remove CNV and Indels, as UM imputation server requires biallelic variants.
...
Using posCmp function from LongevityTools to compare positions from our MrOS SNP annotation with Will Rayner's. Disagreements in chr or position in 7,433 of the 907,956 probes (.8%). Downstream steps rely on Imputation checking tool based on Will Rayner's annotation, so will use his chr and pos.
...
Using strandFlip function from LongevityTools, flip strand alleles are represented on for minus strand SNPs. After flipping minus strand SNPs, check the SNPs again to make sure none are flagged as being on the minus strand. None are flagged. 
...
Generates plink scripts to flip strands and switch reference/other allele. 


A few steps to format the plink files for the UM imputation server.


Files back. Need to convert from vcf to dosage. 
```
#!/bin/bash
...
Tried loading in PLINK, but errors, reporting duplicate IDs. Turns out InDels have same SNPIDs as SNPs at same position because SNP ID is chr:pos. Strangely, I could not run this in parallel, had to do it serially. 
...
#Only if there were dups in the 100 character subset, create new SNPtest with minimum number of BPs so there are no dups. 
...
#approximate R's memory requirements
sapply(seq(0.5,4,by=0.25 )  ,function(x){cbind(x,x*(13/0.7))} )
