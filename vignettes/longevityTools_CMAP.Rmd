...
needs to be created by the user prior to running the download functions. Since
some of the raw data sets are large, the following will only rerun the download
...
The CMAP data set utilizes three different Affymetrix chip (HG-U133A,
...
The follwoing processes each chip type separately using the MAS5 normalization algorithm. 
To save time, the processing is parallelized with `BiocParallel` to run on a computer cluster
with a scheduler (_e.g._ Torque).

```{r normalize_chips, eval=TRUE}
...
```{r comb_chip_type_data, eval=TRUE}
...
# Version information```{r sessionInfo}
sessionInfo()
```
<div align="right">[Back to Table of Contents]()</div>
