...
MAS5 normalization algorithm. To save time, the processing is parallelized with
`BiocParallel` to run on 100 CPU cores of a computer cluster with a scheduler
(_e.g._ Torque). The number of CEL files from each chip type are: 807 CEL files
from HG-U133A, 6029 CEL files from HT_HG-U133A, and 220 CEL files from
U133AAofAv2. The results are written to 3 subdirectores under `data` that are
named after the chip type names.
...
Note: the file `cmap_instances_02.xls` downloaded from CMAP required some 
cleaning in LibreOffice (Excel would work for this too). After this 
it was saved as tab delimited txt file.
