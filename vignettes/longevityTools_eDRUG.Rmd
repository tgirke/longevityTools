...
DEG sets from CMAP. The result includes the Jaccard index is used as a simple similarity 
metric among gene sets as well as the raw and adjusted p-values based on the hypergeometric 
distribution expressing how likely it is to observe the observed intersect sizes just by 
chance. The 20 top scoring drugs are given below and the full data set is written to a file named 
